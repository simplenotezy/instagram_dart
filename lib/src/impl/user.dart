...
class InstagramUsersApiImpl implements InstagramUsersApi {
  late _InstagramUsersApiSelfImpl? _self;
  final Requestor requestor;

  InstagramUsersApiImpl(this.requestor);

  @override
  InstagramUsersApiSelf get self => _self ??= new _InstagramUsersApiSelfImpl(this, requestor);

  @override
  Future<User> getById(String userId) {
    return requestor.request('$_root/$userId').then((r) => new User.fromJson(r.data));
  }
  ...
  Future<List<Media>> getRecentMedia(
    ...
    return requestor.request('$_root/$userId/media/recent', queryParameters: queryParameters).then((r) {
...
class _InstagramUsersApiSelfImpl implements InstagramUsersApiSelf {
  ...
  Future<List<Media>> getLikedMedia({String? maxLikeId, int? count}) {
    ...
    return requestor.request('$_root/self/media/liked', queryParameters: queryParameters).then((r) {
      return r.data.map((m) => new Media.fromJson(m)).toList();
    });
  }
